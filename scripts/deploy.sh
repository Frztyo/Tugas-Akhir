...
# ── Fungsi format durasi milidetik ke detik ────────────────────────────────────
format_duration() {
...
echo "[$(date '+%H:%M:%S')] [FASE 2] Swap dan permission selesai. ($(format_duration "$FASE2_DURATION")s)" >> $LOG_FILE

# ─────────────────────────────────────────────────────────────────────────────
# FASE 3: RELOAD NGINX — Terapkan perubahan tanpa downtime
# ─────────────────────────────────────────────────────────────────────────────
FASE3_START=$(date +%s%3N)
...
echo "[$(date '+%H:%M:%S')] [FASE 3] Nginx berhasil direload. ($(format_duration "$FASE3_DURATION")s)" >> $LOG_FILE

# ─────────────────────────────────────────────────────────────────────────────
# FASE 4: HEALTH CHECK — Verifikasi layanan, rollback jika gagal
# ─────────────────────────────────────────────────────────────────────────────
FASE4_START=$(date +%s%3N)
...
if [ $HEALTH_STATUS -ne 0 ]; then
  ...
  echo "[$(date '+%H:%M:%S')] [ERROR] Health check gagal! ($(format_duration "$FASE4_DURATION")s)" >> $LOG_FILE
  ...
  echo "$(date '+%Y-%m-%d %H:%M:%S') | STATUS=GAGAL | BRANCH=$BRANCH | COMMIT=$COMMIT_SHA | TOTAL=$(format_duration "$TOTAL_DURATION")s | F1=$(format_duration "$FASE1_DURATION")s | F2=$(format_duration "$FASE2_DURATION")s | F3=$(format_duration "$FASE3_DURATION")s | F4=$(format_duration "$FASE4_DURATION")s" >> $RECORD_FILE

  echo "============================================================" >> $LOG_FILE
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] DEPLOYMENT GAGAL — Total: $(format_duration "$TOTAL_DURATION")s" >> $LOG_FILE
  echo "============================================================" >> $LOG_FILE

  exit 1
fi

echo "[$(date '+%H:%M:%S')] [FASE 4] Health check berhasil. ($(format_duration "$FASE4_DURATION")s)" >> $LOG_FILE

# ── Catat total waktu dan rekaman sukses ───────────────────────────────────────
DEPLOY_END=$(date +%s%3N)
TOTAL_DURATION=$((DEPLOY_END - DEPLOY_START))

echo "$(date '+%Y-%m-%d %H:%M:%S') | STATUS=SUKSES | BRANCH=$BRANCH | COMMIT=$COMMIT_SHA | TOTAL=$(format_duration "$TOTAL_DURATION")s | F1=$(format_duration "$FASE1_DURATION")s | F2=$(format_duration "$FASE2_DURATION")s | F3=$(format_duration "$FASE3_DURATION")s | F4=$(format_duration "$FASE4_DURATION")s" >> $RECORD_FILE

echo "============================================================" >> $LOG_FILE
echo "[$(date '+%Y-%m-%d %H:%M:%S')] DEPLOYMENT BERHASIL ✅" >> $LOG_FILE
echo "  Total waktu : $(format_duration "$TOTAL_DURATION") detik" >> $LOG_FILE
echo "  Rincian     :" >> $LOG_FILE
echo "    Fase 1 Backup       : $(format_duration "$FASE1_DURATION")s" >> $LOG_FILE
echo "    Fase 2 Swap Berkas  : $(format_duration "$FASE2_DURATION")s" >> $LOG_FILE
echo "    Fase 3 Reload Nginx : $(format_duration "$FASE3_DURATION")s" >> $LOG_FILE
echo "    Fase 4 Health Check : $(format_duration "$FASE4_DURATION")s" >> $LOG_FILE
echo "============================================================" >> $LOG_FILE